...
router.post("/upload2", upload.single("image"), async (req, res) => {
  ...
  if (!user) return res.status(400).send({ error: "Phn no is incorrect." });
...
router.post("/register2", async (req, res) => {
  ...
  if (emailExist) return res.status(400).send({ error: "phn already exists" });
...
router.post("/allFriends", async (req, res) => {
  try {
    ...
    console.log("error in allFriends api :",e)
    res.status(400).send();
  }
});
...
router.post("/getnames", async (req, res) => {
  try {
    ...
    console.log("error in getnames ", e);
    res.status(400).send();
  }
});
